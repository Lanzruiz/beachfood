...
class MenuCategory extends React.Component {
  ...
  loadCategoryData() {
    ...
    let theCategoryData = [];
     ...
     menuCategoryRef.once('value', function (snapshot) {
       console.log("=================");
       ...
       if(!snapshot.exists()) {
         ...
         snapshot.forEach(function(userItem) {
           let categoryRec = userItem.val();
           console.log(categoryRec);
            theCategoryData .push({key: userItem.key, name: categoryRec.name})
            counter = counter + 1;
         });
  ...
  askDeleteConfirm(key) {
      ...
      swal({
          title: "Are you sure?",
          text: "Once deleted, you will not be able to recover this menu category!",
          icon: "warning",
          buttons: true,
          dangerMode: true,
      }).then((willDelete) => {
          if (willDelete) {
              menuCategoryRef.child(key).remove();
          }
      });
  };
    ...
    return (
      ...
      <Grid item xs={12} className="pageToolbarRight">
          <div style={{margin: "20px"}}>
              <Link to={`/menu_category/create`} style={{color: '#FFFFFF', background: '#147dc2', padding: "10px", margin: "10px", textDecorationLine: "none"}} aria-label="Add Restaurant Owner">
                  Add Menu Category
              </Link>
          </div>
          ...
          columns={[
            {
              ...
              columns: [
...

                {
                  ...
                  Cell: row => (
                    <div>
                        <Link to={`/menu_category/edit/`+row.value} style={{color: '#000'}} aria-label="Edit">
                            <EditIcon />
                        </Link>
